...
function class:destruct()
    event.trigger(self, EVENT.Object.Destruct, { triggerObject = self })
    self:buffClear()
    ...
    local evt = oop._i2o[self._id]._evt
    if (type(evt) == "table") then
        for _, v in pairx(evt) do
            destroy(v)
        end
    end
...
function class:propChange(key, space, setVal, isExec)
    ...
    if (updated) then
        local checkChange = event.checkPropChange(key)
        if (true == checkChange) then
            event.trigger(self, EVENT.Prop.BeforeChange, {
                triggerObject = self,
                key = key,
                old = oldVal,
                new = newVal,
        ...
        if (true == checkChange) then
            event.trigger(self, EVENT.Prop.Change, {
                triggerObject = self,
                key = key,
                old = oldVal,
                new = newVal,
...
function class:PROP(key, variety, duration)
    ...
    if (aid == "0") then
        ...
        if (duration <= 0) then
            desc[#desc + 1] = "持续: " .. colour.hex(colour.gold, "常驻效果")
        else
